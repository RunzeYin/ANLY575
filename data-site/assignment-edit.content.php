...
if (isset($_POST['id'])) {
	...
	$updateSql = "UPDATE `assignments` SET `name` = \"{$postname}\", `description` = \"{$postdescription}\", `course_id` = \"{$postcourse_id}\", `deadline` = \"{$postdeadline}\" WHERE `id` = " . $_GET['id'];
...
$form .= "<p><label for=\"course_id\">Course ID</label> <input type=\"text\" name=\"course_id\" id=\"course_id\" value=\"{$data->course_id}\"></p>";
$form .= "<p><label for=\"deadline\">Deadline</label> <input type=\"text\" name=\"deadline\" id=\"deadline\" value=\"{$data->deadline}\"></p>";
